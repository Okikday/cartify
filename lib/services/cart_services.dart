    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        return response.data['message'] ??
            "Product is currently out of stock or insufficient stock";
      } else {
  ...
  Future<List?> getUserCarts() async {
    ...
    if (role == null) {
      if (globalNavKey.currentContext!.mounted)
        DeviceUtils.showFlushBar(globalNavKey.currentContext!,
            "User role not set. Try logging in again");
      return null;
    }
    ...
    if (apiKey == null) {
      if (globalNavKey.currentContext!.mounted)
        DeviceUtils.showFlushBar(globalNavKey.currentContext!,
            "Account error. Try logging out and in again");
      return null;
    }
    ...
    try {
      ...
      if (response.statusCode == 200 || response.data['success'] == true) {
        ...
        if (globalNavKey.currentContext!.mounted)
          DeviceUtils.showFlushBar(globalNavKey.currentContext!,
              "User hasn't added product to Carts");
        return null;
      } else {
        if (globalNavKey.currentContext!.mounted)
          DeviceUtils.showFlushBar(
              globalNavKey.currentContext!, "Unable to load cart");
        return null;
      }
    } catch (e) {
      if (globalNavKey.currentContext!.mounted)
        DeviceUtils.showFlushBar(globalNavKey.currentContext!, e.toString());
      return null;
    }
  }
  ...
  Future<String?> removeProductFromCarts({
    ...
    try {
      final Response response = await dio.delete(
        ...
        queryParameters: {'version': "v1", 'productId': productId},
